...
from tests.testapp.serializers import PostOutputSerializer


class TestTwoWayInputView(APITestCase):
    ...
    def test_input(self):
        data = {"title": "post title",
                ...
                "blog": self.blog.id,
                "author": self.user.id}
        response = self.client.post(reverse('post_list_create'), data=data, format='json')
...
class TestTwoWayOutputView(APITestCase):
    def setUp(self):
        self.client = APIClient()

        self.blog = Blog.objects.create(name="blog1")
        self.user = User.objects.create_user(username="user1", password="password")
        self.post01 = Post.objects.create(title="post title", content="post content", blog_id=self.blog.id,
                                          author_id=self.user.id)
        self.post02 = Post.objects.create(title="post title", content="post content", blog_id=self.blog.id,
                                          author_id=self.user.id)
